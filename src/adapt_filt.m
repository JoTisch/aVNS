function result = adapt_filt(insp,resp_tR,tsignal,RR,meanF,q)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%Description: 
%
%%INPUT: 
...
for k = 1:5
    af_RRi = interp1(resp_tR,RR{k,1},tsignal,'linear');
    n = round(max(tsignal)*q);
    ti = linspace(0,max(tsignal),n);
    RRi = interp1(tsignal,af_RRi,ti,'linear');
    RRi(isnan(RRi)) = 1; 
    for j= 0:length(insp)
        %% FIRST INTERVAL
        if j == 0 
        ...
        middle(segment) = filt_RR(segment); 
        
    end 
    result{k,1} = interp1(tsignal,middle,resp_tR,'linear','extrap'); 
end 
%% PLOT 
% if pltflg == 1 
%     figure()
